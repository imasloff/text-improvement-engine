...
def calculate_similarity(input_phrases: List[str], standard_terms: List[str], model: SentenceTransformer) -> Tuple[str, float]:
    ...
    similarity_scores = util.pytorch_cos_sim(input_embeddings, standard_embeddings).flatten()
    most_similar_index = similarity_scores.argmax()
    return standard_terms[most_similar_index], similarity_scores[most_similar_index]
    ...
    for input_phrase in phrases_to_check:
      ...
      if similarity_score > threshold:
        if input_phrase in suggestions:
          suggestions[input_phrase] = max(suggestions[input_phrase], (best_term, similarity_score), key=lambda x: x[1])
        else:
